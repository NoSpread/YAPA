...
import {
    Weather,
    IDaySelected,
    IDayForecastSelected,
    ICurrWeatherSelected
} from './IWeather'
...
class WeatherModel {
    ...
    public getCurrentWeather = async (location: string): Promise < ICurrWeatherSelected > => {
        ...
        const result = await got < Weather > (this.endpoint + "current.json", {
            searchParams: {
                key: this.apikey,
                q: location,
                aqi: "no"
    ...
    public getForecastWaether = async (location: string, days: number): Promise < IDayForecastSelected > => {
        ...
        const result = await got < Weather > (this.endpoint + "forecast.json", {
            searchParams: {
                key: this.apikey,
                q: location,
                aqi: "no",
